...
class LoginView(APIView):
    ...
    Note:-
    Checks whether the request is from user
    by checking the credentials and if that entry is primary.
    """
...
class UserUpdatePassword(APIView):
    ...
    def post(self, request, format=None):
        ...
        error_checks = system_error.check_for_update_password_input_error(request.data)
        if error_checks:
            return Response(error_checks,
                            status=status.HTTP_412_PRECONDITION_FAILED)
...
class UserLogout(APIView):
    """
    API to logout a User Revoking all its access rights
    """
